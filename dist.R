...
lpz.dist <- function(Z) {
  # gives the negative distances between nodes
  ZtZ <- Z%*%t(Z)
...
proc.crr <- function(Z, Z0, k=2) {
  # Procrustes transform; gives rotation, reflection, translation of Z closest to Z0
  for (i in 1:k) {
...
Z.up <- function(Y, Z, alpha, zdelta, mu.z=0, sd.z=10) {
  ...
  hr <- lnew - lold + sum(dnorm(Znew, mu.z, sd.z, log=TRUE)) - sum(dnorm(Z, mu.z, sd.z, log=TRUE))
