setwd("C:/Users/sangwonhan/projects/latent_space_models")
...
avZ <- avZ*2/(avZ[1]) # MLE too extreme, keep share but rescale
alpha <- avZ[1]
...
## MCMC params
adelta <- 0.5           # params for
zdelta <- 0.2           # proposal distribution
...
plot(Z.mle[, 1], Z.mle[, 2], xaxt="n", yaxt="n", xlab="", ylab="", type="n", xlim=range(Zp[[1]]), ylim=range(Zp[[2]]))
...
for (i in 1:n) {
  points(Zp[[1]][, i], Zp[[2]][, i], pch=46, cex=0.75, col=rgb(r[i], g[i], b[i]))
}
